...
VOL = os.path.join("../..", "run3", "flow_plane.vtu")
SUR = os.path.join("../..", "run3", "surface_flow_plane.vtu")
...
save_pic(wing, "Pressure", "wing_pressure_plane.png")
if "Cp" in wing.array_names:
    save_pic(wing, "Cp", "wing_cp_plane.png")
...
if "|U|" in sampled.array_names:
    save_pic(sampled, "|U|", "wing_nearwall_speed_plane.png")
...
save_pic(slice_x, fld, "slice_plane.png", overlay=wing)
...
if "Velocity" in vol.array_names:
    ...
    try:
        ...
        p.add_axes()
        p.show(screenshot="streamlines_plane.png")
        p.close()
